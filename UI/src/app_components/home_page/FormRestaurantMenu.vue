        ...
        <MLVbox style="width: 100%; height: 75%; overflow: hidden;" align="center" class="sm:h-fit">
            ...
            <MLVbox v-if="displayMenuType === 1" style="width: 100%; overflow-y: auto;" class="mt-10">
                <MLVbox v-for="category in lstDisplayItems" class="mb-12">
                    <h3 class="text-2xl font-semibold text-emerald-600 mb-6 pb-2 border-b-2 border-emerald-200">
                        {{ category.MenuItemCategoryName }}
                    </h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-1 px-1">
                        <div v-for="menuItem in category.MenuItems" class="bg-white pa-3 md:p-4 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 flex flex-col md:flex-row gap-3 md:gap-4 group">
                            <div class="w-full h-48 md:w-24 md:h-24 rounded-lg overflow-hidden flex-shrink-0">
                                <img
                            ...
                            <div class="flex-1">
                                <div class="flex justify-between items-start">
                                    <h4 class="font-medium text-gray-800 group-hover:text-emerald-600 transition-colors">
                                        <b>{{ menuItem.Name }}</b>
                                    </h4>
                                    <p class="font-semibold text-emerald-600">{{ $commonFunction.formatThousands(menuItem.Price) }} đ</p>
                                </div>
