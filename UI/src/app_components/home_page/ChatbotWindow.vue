<template>
    <div class="chat-button-container">
        <VBtn @click="toggleChatbotWindow" icon="mdi-chat" color="primary" />

        <VCard v-show="showWindow" style="position: absolute; right: 100%; width: 328px; height: 455px; margin-right: 16px; bottom: 0; display: flex; flex-direction: column;">
            ...
            <MLVbox ref="messages" style="flex-grow: 1; padding: 0.625rem 0.5rem; overflow-y: auto;">
                <template v-for="detail, index in conversation?.ChatbotConversationDetails">
                    ...
                    <MLHbox :class="[
                        ...
                        <VCard rounded style="padding: 0.3rem 0.7rem;">{{ detail.Message }}</VCard>
                    </MLHbox>
                </template>
            ...
            if (this.messageInput && this.conversation) {
                ...
                try {
                    ...
                    if (detail) {
                        ...
                        const responseDetail:ChatbotConversationDetail|undefined = await this.$service.ChatbotService.getNewResponse();
                        if (responseDetail) {
