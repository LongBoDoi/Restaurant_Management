<template>
    <VCard class="bg-gray-50 rounded-xl pa-6 shadow-md hover:shadow-lg transition duration-300 border-l-4 border-blue-500" :disabled="loading">
        ...
        <div class="flex justify-between h-full space-x-3">
            <div>
                <p class="text-gray-500 text-sm">Tổng số order</p>
                <div v-if="!loading">
...
export default {
    ...
    methods: {
        async loadData(data: any) {
            ...
            const objectData = await this.$service.DashboardService.getTotalOrders(data.FromDate, data.ToDate, data.TimeFilter);
